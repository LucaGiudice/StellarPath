...
#' Install StellarPath required python packages in conda or virtualenv environment
#'
...
#'
#' @description Install StellarPath required python packages (rpp) in a self-contained environment.
#' For macOS and Linux-based systems, this will also install Python itself via a "miniconda" environment, for
#'   \code{SP_install}.  Alternatively, an existing conda installation may be
...
#' @param envname character; name of the conda-environment to install StellarPath required python packages.
#'   Default is "SP_condaenv".
#' @param prompt logical; ask whether to proceed during the installation
#' @examples
#' \dontrun{
#' # install StellarPath required python packages in a miniconda environment (macOS and Linux)
#' SP_install(prompt = FALSE)
#'
#' # install StellarPath required python packages to an existing conda environment
#' SP_install(conda = "~/anaconda/bin/")
#' }
#' @export
SP_install <- function(conda = "auto",
                            update_conda = FALSE,
                            force_conda = FALSE,
  ...
  if (.Machine$sizeof.pointer != 8) {
    stop(
      "Unable to install the StellarPath-package on this platform.",
  ...
  if (is_unix()) {
    ...
    if (!have_conda) {
      ...
      if (prompt) {
        ans <- utils::menu(c("No", "Yes"), title = "Do you want StellarPath to download
    ...
    # process the installation of StellarPath required python packages
    process_SP_installation_conda(conda,
      rpp_version,
      python_version,
      prompt,
      envname = envname,
    ...
    if (python_version == "find_python") {
      ...
      python_versions <- python_versions[python_versions$version %in% c("3.7", "3.8", "3.9"), ]
...
process_SP_installation_conda <- function(conda,
                                               rpp_version,
                                               python_version,
                                               prompt = TRUE,
                                               envname = "SP_condaenv",
  ...
  if (nrow(conda_env) == 1) {
    cat(
      "Using existing conda environment ", envname, " for StellarPath installation\n.",
      "\nStellarPath:",
    ...
    cat("Creating", envname, "conda environment for StellarPath installation...\n")
  ...
  cat("Installing StellarPath required python packages...\n")
...
#'   \code{SP_install_virtualenv} function. It requires that you have a python version
#'   and path to it (such as "/usr/local/bin/python3.9" for Mac and Linux.).
#' @param pip_version character;
#' @examples
#' \dontrun{
#' # install StellarPath required python packages in a virtual environment
#' SP_install_virtualenv()
#' }
#' @export
SP_install_virtualenv <- function(      rpp_version = c("protobuf==3.19.4",
                                                        "chardet==3.0.4",
                                                        "scikit-learn==1.0.2",
                                                        "pandas==1.4.1",
                                                        "networkx==2.7.1",
  ...
  process_SP_installation_virtualenv(
    python = python,
...
process_SP_installation_virtualenv <- function(python = "/usr/local/bin/python3.8",
                                                    rpp_version,
                                                    pip_version,
                                                    envname = "SP_virtualenv",
                                                    prompt = TRUE) {
  ...
  cat(sprintf(
    ...
    the following StellarPath reuired python packages will be installed: \n "%s" \n \n',
...
# For example, bin = "pip3" bin = "python3.8" bin = ".virtualenv"
...
#' Uninstall StellarPath conda environment
#'
#' Removes the conda environment created by SP_install()
...
#' @export
SP_uninstall <- function(conda = "auto",
                              prompt = TRUE,
                              envname = "SP_condaenv") {
  conda <- tryCatch(reticulate::conda_binary(conda), error = function(e) NULL)
  have_conda <- !is.null(conda)
