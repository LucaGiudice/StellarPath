...
#'
#' Determine which is the pairwise similarity between column profiles based on Adjusted Weighted Jaccard similarity.
...
#'
#' @param m Numeric matrix s.t. columns are objects, rows are features and a cell contains a value
#' @param up Default TRUE, boolean that defines if the significant molecules are upregulated in a patient's group.
#' @param comb2 Default NULL, dataframe of iterator, index 1 and index 2 refering to the pair of columns to compute
...
#' @import stats
#' @export
build_adjWJ_PSN <- function(m, up=TRUE, comb2=NULL) {
  if(is.null(comb2)){
    samples <- 1:ncol(m)
  ...
  if(up){
    m=scales::rescale(m,c(0.1,0.9))
  }else{
    m=scales::rescale(m,c(0.9,0.1))
  }
...
#'
#' For each pair of column profiles, it determines the average of the feature's values and then
#' it determines their average to assign the score of similarity to the pair of profiles.
#'
#' @param m Numeric matrix s.t. columns are objects, rows are features and a cell contains a value
...
#' Create Patient Similarity Network with StellarPath's similarity
#'
#' It creates the PSN based on the Adjusted Weighted Jaccard similarity.
#' It creates the PSN based on the Average of Pair Weights.
#' In case of upregulated significant molecules populating the count matrix, then more
#' the average is high and more the similarity is high.
#' In case of downregulated significant molecules populating the count matrix, then more
#' the average is low and more the similarity is high.
#' It combines the two values that each pair of samples has.
#' The combination is performed computing the triangular area of the two values of similarity.
#'
#' @param m Numeric matrix s.t. columns are objects, rows are features and a cell contains a value
#' @param up Default TRUE, boolean that defines if the significant molecules are upregulated in a patient's group.
...
build_PSN=function(m, up=TRUE){
  ...
  adjWJ_PSN=build_adjWJ_PSN(m,up,comb2)
  avg_PWN=build_avg_PWN(m,comb2)

  adjWJ_PSN=scales::rescale(adjWJ_PSN,to = c(0.1,1))
  avg_PWN=scales::rescale(avg_PWN,to = c(0.1,1))
  ...
  if(up){
    ...
    score_PSN_high = scales::rescale(score_PSN_high, to=c(0.1,1))
    diag(score_PSN_high)=1
    return(score_PSN_high)
  }else{
    ...
    score_PSN_low = scales::rescale(score_PSN_low, to=c(0.1,1))
    diag(score_PSN_low)=1
    return(score_PSN_low)
  }
}
